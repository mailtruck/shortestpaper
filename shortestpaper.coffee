...
(($) ->
  # Grab all the hashes of the URLs
  keys = [];
  $('#bookmark_list .tableViewCellTitleLink').each () ->
    key = Crypto.SHA1(this.href)[0..9]
    $(this).closest('.tableViewCell').data('key', key)
    keys.push(key)

  $.getJSON '/counts.json', { keys: keys.join() }, (json) ->
    bml = $('#bookmark_list')
    stories = []
    $('.tableViewCell', bml).each () ->
      t = $(this)
    ...
    stories.sort (l, r) ->
      ...
      json[$(l).data('key')] - json[$(r).data('key')]
