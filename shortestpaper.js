...
(function($) {
  var keys;
  keys = [];
  $('#bookmark_list .tableViewCellTitleLink').each(function() {
    var key;
    key = Crypto.SHA1(this.href).slice(0, 9 + 1);
    $(this).closest('.tableViewCell').data('key', key);
    return keys.push(key);
  });
  return $.getJSON('/counts.json', {
    keys: keys.join()
  }, function(json) {
    var bml, stories;
    bml = $('#bookmark_list');
    ...
    $('.tableViewCell', bml).each(function() {
      var count, t;
      t = $(this);
      count = json[t.data('key')];
      if (typeof count !== "undefined" && count !== null) {
        $('.secondaryControls', t).append("<span class='host'>" + (count) + " words</span>");
      }
      t.removeClass('tableViewCellLast tableViewCellFirst');
      return stories.push(this);
    });
    stories.sort(function(l, r) {
      return json[$(l).data('key')] - json[$(r).data('key')];
    });
    bml.empty();
    $.each(stories, function() {
      return bml.append($(this));
    });
    $('.tableViewCell:first', bml).addClass('tableViewCellFirst');
    return $('.tableViewCell:last', bml).addClass('tableViewCellLast');
  });
})(jQuery.noConflict());